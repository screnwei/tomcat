...
public final class Bootstrap {
    ...
    private ClassLoader createClassLoader(String name, ClassLoader parent)
        ...
        // 替换 catalina.home、或者catalina.base，或者环境变量
        value = replace(value);
        ...
        for (String repository : repositoryPaths) {
            ...
            // Local repository
            if (repository.endsWith("*.jar")) {	// 以 *.jar结尾，删除通匹字符
    ...
    public void init() throws Exception {
        ...
        // 反射方法实例化Catalina，利用类加载器加载类，而不是使用new关键字，主要是考虑需要从catalina类加载器加载class
   ...
   /**
    ...
    public static void main(String args[]) {
        ...
        // 根据命令行传递的参数，调用Catalina的start、stopServer方法，默认是调用start方法
        try {
            ...
            // 1、启动时，Catalina.setAwait(true)，其目的是为了让tomcat在关闭端口阻塞监听关闭命令，参考Catalina.await()方法
            // 2、deamon.load(args)，实际上会去调用Catalina#load(args)方法，会去初始化一些资源，优先加载conf/server.xml，找不到再去加载server-embed.xml
            // 		找不到xml也没关系，因为很多内嵌的tomcat不需要xml配置
            //		此外，load方法还会初始化Server
            // 3、daemon.start()，实例上会去调用Catalina.start()
